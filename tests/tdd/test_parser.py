...
class TestParser(unittest.TestCase):
    def test_parse_rub_for_usd_exchange_rate(self):
        parser = Parser()
        result = asyncio.run(parser.getExchangeRate('USD', 'RUB', '10.10.2023'))
        expected = 101.3598
    ...
    def test_parse_usd_for_rub_exchange_rate(self):
        parser = Parser()
        result = asyncio.run(parser.getExchangeRate('RUB', 'USD', '10.10.2023'))
    ...
    def test_parse_same_currency(self):
        parser = Parser()
        result = asyncio.run(parser.getExchangeRate('RUB', 'RUB', '10.10.2023'))
        expected = 1
        self.assertEqual(expected, result)
